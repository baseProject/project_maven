...
<body>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%"
		...
		<c:forEach items="${pageView.data}" var="item" varStatus="varStatus">
			<tr onmousemove="javascript:this.bgColor='#FCFDEE';" onmouseout="javascript:this.bgColor='#FFFFFF';">
				...
				<td>
					<c:if test="${item.menu==false }">
						是
					</c:if>
					<c:if test="${item.menu==true }">
						否
					</c:if>
				</td>
				<td><c:out value="${item.pId }"></c:out></td>
				<td>
					<fmt:formatDate value="${item.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<input type="button" value="修改" onclick="edituserDialog(${item.id})"/>
					<input type="button" value="修改密码" onclick="updatePS(${item.id})"/>
	...
	<div class="pages">
		<c:import url="../common/pagnation.jsp">
	         ...
	         <c:param name="currentFormId" value="listResources"/>
	    </c:import>
	</div>
	</form>
</body>
